...
import sys
base = '/media/bit301/data/yml/project/python310/p3/DATASET2aa'
...
class nnUNetPredictor(object):
    ...
    def predict_single_npy_array(self, input_image: np.ndarray, image_properties: dict,label:np.ndarray,
        ...
        """
        input_image=np.concatenate([input_image,label],axis=0)
        ...
        # ma=mask1=data[0].max()
        # mi = mask1 =data[0].min()
        # ma2 = mask1 = data[1].max()
        # mi2 = mask1 = data[1].min()
        data = dct['data']
        binary_mask_rounded = (torch.round(data[1, :, :, :])).type(torch.uint8).unsqueeze(0)
        data_with_mask = data[0,:,:,:].unsqueeze(0) * copy.deepcopy(binary_mask_rounded)
        data = torch.cat((data[0,:,:,:].unsqueeze(0), data_with_mask,binary_mask_rounded), dim=0)
        ...
        if output_file_truncated is not None:
            ...
            # _, ret = torch.max(predicted_logits.data, dim=0)#1
...
def remove_small_volums(mask):
    ...
    for i in range(1, label_shape_filter.GetNumberOfLabels() + 1):
        if label_shape_filter.GetNumberOfPixels(i) >= 1000:
...
def crop_by_pseg(ncct_npy,pseg_npy,seg_npy):
    output_size = [1000, 300, 256]  # 统计mask范围得到my=262 mx=217
    index = np.nonzero(pseg_npy)
    # 检查是否有非零值
    if index[1].size == 0:
        ...
        x_min = np.min(index[3])
        x_max = np.max(index[3])
    ...
    seg_npy = seg_npy[:, :,crop_y_down: crop_y_up, crop_x_down: crop_x_up]
    pseg_npy = pseg_npy[:,:, crop_y_down: crop_y_up, crop_x_down: crop_x_up]
    ncct_npy = ncct_npy[:,:, crop_y_down: crop_y_up, crop_x_down: crop_x_up]
    return ncct_npy,pseg_npy,seg_npy
...
class Inference2aa():
    ...
    def predict1(self, input_dir, output_dir):
        path_list = []
        # inputs = input_dir  # 输入目录
        # inputs = "/media/bit301/data/yml/data/p3/external/hnnk/"  # pre
        inputs = "/media/bit301/data/yml/data/p3_crop_preseg/external/"  # external
        ...
        ij = 0
        # models=['nnUNetTrainer']
        pre_seg1 = "test2a/p3t/" + self.model
        for path in path_list:
            # if "nnUNetTrainerSegMamba"==self.model:
            #     if ij < 560:
            #         ij = ij + 1
            #         continue
            ncct_npy, props = SimpleITKIO().read_images([path])
            #preseg_path = path.replace("p3_crop_by2a", pre_seg1) #因为有很多组织分割，使用低分辨率预分割裁减。
            preseg_path=path.replace("0.nii.gz","22.nii.gz")
            pseg_npy, label_props = SimpleITKIO().read_images([preseg_path])
            # mask1 = sitk.GetArrayFromImage(sitk.ReadImage(out_put))
            ret = self.predictor.predict_single_npy_array(ncct_npy, props, pseg_npy, None, None, False)
            # ret = remove_regions(ret)  #直接计算最终的结果
            ret = remove_small_volums(ret)  #直接计算最终的结果

            label = sitk.GetImageFromArray(ret.astype(np.int16))
            out="test2aa/p3ta/"+self.model# flip
            out_put = path.replace("p3_crop_preseg", out).split("0.nii.gz")[0]
    ...
    def predict2(self, input_dir, output_dir):
        path_list = []
        # inputs = input_dir  # 输入目录
        # inputs = "/media/bit301/data/yml/data/p3/external/hnnk/"  # pre
        inputs = "/media/bit301/data/yml/data/p3_crop_preseg/external/"  # pre
        for root, dirs, files in os.walk(inputs, topdown=False):
            for file in files:
                path = os.path.join(root, file)
                if "0.nii.gz" in path:
        ...
        for path in path_list:
            ncct_npy, props = SimpleITKIO().read_images([path])
            #preseg_path = path.replace("p3_crop_preseg", pre_seg1) #因为有很多组织分割，使用低分辨率预分割裁减。
            preseg_path=path.replace("0.nii.gz","22.nii.gz")
            pseg_npy, label_props = SimpleITKIO().read_images([preseg_path])
            # mask1 = sitk.GetArrayFromImage(sitk.ReadImage(out_put))
            ret = self.predictor.predict_single_npy_array(ncct_npy, props, pseg_npy, None, None, False)
            ...
            label = sitk.GetImageFromArray(ret.astype(np.int16))
            out="test2aa/p3tb/"+self.model# flip
            out_put = path.replace("p3_crop_preseg", out).split("0.nii.gz")[0]
            if not os.path.isdir(out_put):
                os.makedirs(out_put)
            file_path = os.path.join(out_put, "2.nii.gz")
...
if __name__ == '__main__':
    ...
    # id=0
    # model="nnUNetTrainerAortaNet1"#nnUNetTrainer nnUNetTrainerMaCNN
    inference = Inference2aa(id,model)
    input_dir = "a"
    output_dir="b"
    inference.predict1(input_dir, output_dir)#test2aa:flip
    # inference.predict2(input_dir, output_dir)#test2ab:false lume