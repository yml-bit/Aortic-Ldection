...
base = '/media/bit301/data/yml/project/python310/p3/DATASET2a'
...
def remove_small_volums(mask):
    ...
    for i in range(1, label_shape_filter.GetNumberOfLabels() + 1):
        if label_shape_filter.GetNumberOfPixels(i) >= 1000:#1000 14000
...
class Inference2al():
    def __init__(self,id:int,
        ...
        self.model=model
        #可以自主选择 96x160x160 80x224x192
        model_path = 'Dataset301_Aorta/' + self.model + '__nnUNetPlans__3d_lowres'  # nnUNetTrainerMaCNN2__nnUNetPlans__3d_lowres
    ...
    def predict(self, input_dir, output_dir):
        ...
        inputs = "/media/bit301/data/yml/data/p3/"  # pre external
        # inputs="/media/bit301/data/yml/data/p3/internal/xy/xy3/dis/jc/PA9_zj"
        ...
        for path in path_list:
            ...
            ret=remove_small_volums(ret)
            label = sitk.GetImageFromArray(ret.astype(np.int16))
            out="test2a/p3t_lowers/"+self.model# p3ta:内膜片 p3tb：假腔
            out_put = path.replace("p3", out).split("0.nii.gz")[0]
            if not os.path.isdir(out_put):
                os.makedirs(out_put)
            file_path = os.path.join(out_put, "22.nii.gz")#binary mask
...
class Inference2af():
    ...
    def predict(self, input_dir, output_dir):
        ...
        for path in path_list:
            ...
            ret=remove_small_volums(ret)
            label = sitk.GetImageFromArray(ret.astype(np.int16))
            out="test2a/p3t_fullers/"+self.model# p3ta:内膜片 p3tb：假腔
            out_put = path.replace("p3", out).split("0.nii.gz")[0]
            if not os.path.isdir(out_put):
                os.makedirs(out_put)
            file_path = os.path.join(out_put, "22.nii.gz")#binary mask
...
def combine():
    path_list = []
    # inputs = input_dir  # 输入目录
    inputs = "/media/bit301/data/yml/data/test2a/p3t_lowers/"  #
    # inputs = "/media/bit301/data/yml/data/test2a/p3t_lowers/nnUNetTrainerMaCNN2/internal/xy/xy3/dis/jc/PA9_zj"
    # inputs="/media/bit301/data/yml/data/p3/external/cq/dis/dml/PA19"###升主动脉
    for root, dirs, files in os.walk(inputs, topdown=False):
        for file in files:
            path = os.path.join(root, file)
            if "22.nii.gz" in path:
                path_list.append(path)
    ij = 0
    for path in path_list:
        readl = sitk.ReadImage(path, sitk.sitkInt16)
        imagel = sitk.GetArrayFromImage(readl)
        readf = sitk.ReadImage(path.replace("p3t_lowers","p3t_fullers"), sitk.sitkInt16)
        imagef = sitk.GetArrayFromImage(readf)
        ret = imagel+imagef
        ret[ret>0]=1
        label = sitk.GetImageFromArray(ret.astype(np.int16))
        file_path = path.replace("p3t_lowers","p3t")
        out_put = file_path.split("22.nii.gz")[0]
        if not os.path.isdir(out_put):
            os.makedirs(out_put)
        sitk.WriteImage(label, file_path)
...
if __name__ == '__main__':
    ...
    # id=0
    # model="nnUNetTrainerMaCNN2"#nnUNetTrainer nnUNetTrainerMaCNN nnUNetTrainerAortaNet1
    input_dir = "a"
    output_dir="b"
    inferencel = Inference2al(id,model)
    inferencel.predict(input_dir, output_dir)#该nnunet模型主要用于定位
    inferencef = Inference2af(id, model)
    inferencef.predict(input_dir, output_dir)  #该nnunet模型主要用于定位
    combine()