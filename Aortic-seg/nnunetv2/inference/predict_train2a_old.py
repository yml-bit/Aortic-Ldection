...
# base = join(os.sep.join(__file__.split(os.sep)[:-5]), 'DATASETT12')
...
if __name__ == '__main__':
    ...
    predictor.initialize_from_trained_model_folder(
        join(nnUNet_results, 'Dataset301_Aorta/nnUNetTrainerunet__nnUNetPlans__3d_fullres'),
        use_folds=(0,),
    ...
    path_list = []
    #nnUNet_raw
    inputs = join(nnUNet_raw, 'Dataset301_Aorta/imagesTr')
    for root, dirs, files in os.walk(inputs, topdown=False):
        for file in files:
            path = os.path.join(root, file)
            if "_0000.nii.gz" in path:
                # filee = root.split("/")[-1]
    ...
    for path in path_list:
        ...
        out_put = path.replace("_0000.nii.gz", "_0001.nii.gz")  # lo:97  x1:54  xlo2:10
