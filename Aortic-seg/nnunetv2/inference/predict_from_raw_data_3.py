...
import sys
base = '/media/bit301/data/yml/project/python310/p3/DATASET3'
...
class Inference():
    def __init__(self,id:int,
        ...
        model_path = 'Dataset301_Aorta/' + 'nnUNetTrainerMaCNN2__nnUNetPlans__3d_fullres' #self.model +
    ...
    def predict1(self, input_dir, output_dir):
        ...
        pre_seg1 = "test2aa/p3ta/" + self.model
        # pre_seg1 = "test2aa/p3tb/" + self.model
        for path in path_list:
            ncct_npy, props = SimpleITKIO().read_images([path])
            preseg_path = path.replace("p3_crop_preseg", pre_seg1) #p3-p3t pp3-pp3t
            preseg_path=preseg_path.replace("0.nii.gz","2.nii.gz")
            ...
            out="test3/p3ta/"+self.model# p3-p3t pp3-pp3t
    ...
    def predict2(self, input_dir, output_dir):
        ...
        # inputs = input_dir  # 输入目录
        # inputs = "/media/bit301/data/yml/data/p3/external/hnnk/"  # pre
        inputs = "/media/bit301/data/yml/data/p3_crop_preseg/external/hnnk/"  # pre
        for root, dirs, files in os.walk(inputs, topdown=False):
            for file in files:
                path = os.path.join(root, file)
                if "0.nii.gz" in path:
        ...
        for path in path_list:
            ...
            label = sitk.GetImageFromArray(ret.astype(np.int16))
            out="test2/p3tb/"+self.model# p3-p3t pp3-pp3t
            out_put = path.replace("p3_crop_preseg", out).split("0.nii.gz")[0]
            if not os.path.isdir(out_put):
                os.makedirs(out_put)
            file_path = os.path.join(out_put, "2.nii.gz")
...
if __name__ == '__main__':
    ...
    # id=0
    # model="nnUNetTrainerMaCNN2"#nnUNetTrainer nnUNetTrainerMaCNN
    inference = Inference(id,model)
    input_dir = "a"
    output_dir="b"
    inference.predict1(input_dir, output_dir)#ba-flip
    # inference.predict2(input_dir, output_dir)#bb-false lume