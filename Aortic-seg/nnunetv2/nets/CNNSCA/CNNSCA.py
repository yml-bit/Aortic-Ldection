...
class MambaLayer(nn.Module):
    def __init__(self, dim, d_state=16, d_conv=4, expand=2, num_slices=None):
        ...
        # self.norm = nn.LayerNorm(dim).to(self.device)  # 将 LayerNorm 移动到选定设备
...
class MambaEncoder(nn.Module):
    def __init__(self, in_chans=1, depths=[2, 4, 6, 2], dims=[48, 96, 192, 384],
...
class SCA3D(nn.Module):#spatial channel attention
...
class EPA(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, proj_size, num_heads=4, qkv_bias=False,
        ...
        # Temperature parameters for scaling attention scores
        self.temperature = nn.Parameter(torch.ones(num_heads, 1, 1))
        self.temperature2 = nn.Parameter(torch.ones(num_heads, 1, 1))

        # Linear layers for query, key, value_spatial, value_channel
        self.qkvv = nn.Linear(in_channels, out_channels * 4, bias=qkv_bias)

        # Projection matrices E and F for spatial attention
        self.EF = nn.Parameter(torch.empty(out_channels, proj_size))
        nn.init.xavier_uniform_(self.EF)  # Xavier initialization
    ...
    def forward(self, x):
        B, C, D, H, W = x.shape
        assert C == self.in_channels, f"Input channels ({C}) do not match expected input channels ({self.in_channels})"

        # Flatten spatial dimensions
        x = x.view(B, C, -1).transpose(1, 2)  # (B, n_tokens, C)

        # Compute Q, K, Vv, Vc
        qkvv = self.qkvv(x)
        q, k, vv, vc = qkvv.chunk(4, dim=-1)

        # Reshape for multi-head attention
        q = q.view(B, -1, self.num_heads, self.out_channels // self.num_heads).transpose(1, 2)
        k = k.view(B, -1, self.num_heads, self.out_channels // self.num_heads).transpose(1, 2)
        vv = vv.view(B, -1, self.num_heads, self.out_channels // self.num_heads).transpose(1, 2)
        vc = vc.view(B, -1, self.num_heads, self.out_channels // self.num_heads).transpose(1, 2)

        # Compute spatial attention
        q = q * self.temperature
        k = k * self.temperature2
        attn = torch.matmul(q, k.transpose(-2, -1))
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
        out_spatial = torch.matmul(attn, vv)

        # Compute channel attention
        k = k.view(B, -1, self.out_channels)
        vc = vc.view(B, -1, self.out_channels)
        k_projected = torch.matmul(k, self.EF)
        vc_projected = torch.matmul(vc, self.EF)
        attn_channel = torch.matmul(k_projected, vc_projected.transpose(-2, -1))
        attn_channel = attn_channel.softmax(dim=-1)
        attn_channel = self.attn_drop_2(attn_channel)
        out_channel = torch.matmul(attn_channel, vc_projected)

        # Combine spatial and channel attention
        out_spatial = out_spatial.transpose(1, 2).reshape(B, -1, self.out_channels)
        out_channel = out_channel.view(B, -1, self.out_channels)
        out = out_spatial + out_channel

        # Reshape back to original shape
        out = out.view(B, self.out_channels, D, H, W)
        return out

##mamba+medxnet、up concaten。mambaencoder替代原始卷及编码器
class MaCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.outside_block_checkpointing:
            ...
            x = self.iterative_checkpoint(self.bottleneck, outs[3])
            if self.do_ds:
...
#0118 编码层注意力 有效果。zX300x300，有效果。当前效果最佳
class CNNSCA(nn.Module):
    def __init__(self,
                 in_channels: int,
                 n_channels: int,
                 n_classes: int,
        ...
        self.scse3 = SCA3D(16 * n_channels)
