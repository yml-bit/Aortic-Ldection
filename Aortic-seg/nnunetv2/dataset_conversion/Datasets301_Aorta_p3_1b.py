...
def copy_files(src_data_folder: Path, train_dir: Path, labelTr_dir: Path, test_dir: Path,labelTs_dir: Path):
    ...
    train_keys=[]
    test_keys=[]
    output_size = [1000, 300, 300]
    # output_size = [1000, 360, 320]  # 统计mask范围得到my=262 mx=217
    ...
    for path_str in patients_train:#内部数据用于开发，选择一个外部数据作为验证集。
        ...
        if file.name == "0.nii.gz":
            ...
            ncct_itk = sitk.ReadImage(file, sitk.sitkInt16)  # 使用sitk重新保存，这样占用内存小很多
            ncct_npy = sitk.GetArrayFromImage(ncct_itk)
            ncct_output=str(train_dir / f"Aorta_{str(num_cases).zfill(4)}_0000.nii.gz")

            label_path = path_str.replace("0.nii.gz", "3.nii.gz")
            seg_itk = sitk.ReadImage(label_path, sitk.sitkInt16)  # 使用sitk重新保存，这样占用内存小很多
            seg_npy = sitk.GetArrayFromImage(seg_itk)
            seg_output=str(labelTr_dir / f"Aorta_{str(num_cases).zfill(4)}.nii.gz")

            index = np.nonzero(seg_npy) #训练时候使用这个裁减没有问题
            # 检查是否有非零值
            if index[0].size == 0:
                print("No non-zero values in the mask.")
            else:
                # 计算非零值的范围
                # 统计mask范围得到my=262 mx=217   # internlal y=262,x=210; external y=227,x=217
            ...
            sitk.WriteImage(seg_itk_new, seg_output)
            # pseg_itk_new = sitk.GetImageFromArray(pseg_npy)
            # sitk.WriteImage(pseg_itk_new, pseg_output)
            ncct_itk_new = sitk.GetImageFromArray(ncct_npy)
            sitk.WriteImage(ncct_itk_new, ncct_output)
...
if __name__ == "__main__":
    ...
    # input_folder="../../../../../../data/p3/pre/"  #pre
    input_folder="/media/bit301/data/yml/data/p3/internal/"  #相对路径下直接运行，命令行运行需要修改为绝对路径
    dataset_id=301
